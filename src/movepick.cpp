...
#else
	stage = pos.is_in_check() ? EVASION : MAIN_SEARCH;
	ttMove = ttm > MOVE_NONE && pos.move_is_legal(ttm) ? ttm : MOVE_NONE;
	endMoves += (ttMove > MOVE_NONE);
#endif
}
...
MovePicker::MovePicker(const Position& p, Move ttm, const HistoryStats& h, Value th)
  ...
  endMoves += (ttMove != MOVE_NONE);
	...
	ttMove =   ttm > MOVE_NONE
	        && pos.move_is_legal(ttm)
	        && MOVE_CAPTURED(ttm) != EMPTY
	        && pos.see(ttm) > threshold ? ttm : MOVE_NONE;
	endMoves += (ttMove > MOVE_NONE);
#endif
...
#else
	for (auto& m : *this) {
		m.value = (Value) ValuePieceMg[MOVE_CAPTURED(m)];
		if (MOVE_IS_PROMOTION(m))
			m.value += ValuePromotionMg[MOVE_PIECE(m)];
		else
			m.value -= (MOVE_PIECE(m) & 7) - 1;
	}
#endif
...
#else
  for (auto& m : *this)
      m.value =  history[MOVED_PIECE(m)][SquareTo81[MOVE_TOSQ(m)]]
               + (*counterMoveHistory )[MOVED_PIECE(m)][SquareTo81[MOVE_TOSQ(m)]]
               + (*followupMoveHistory)[MOVED_PIECE(m)][SquareTo81[MOVE_TOSQ(m)]];
#endif
}
  ...
  for (auto& m : *this)
	...
	for (auto& m : *this) {
		if (MOVE_CAPTURED(m) != EMPTY)
			m.value = HistoryStats::Max + Value(8 - ((MOVE_PIECE(m) & 7)));
		else
			m.value = history[MOVED_PIECE(m)][SquareTo81[MOVE_TOSQ(m)]];
	}
#endif
...
#else
          if (MOVE_TOSQ(move) == recaptureSquare)
#endif
              return move;
          break;
